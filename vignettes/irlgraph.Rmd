...
* Very important point cells - cells that lie on either side of a boundary between different cost-categories in the underlying cost raster. The definition of a boundary is set based on the user-set `cutoff` parameter.
...
```{r eval=TRUE, fig.align='right', fig.height=4, fig.width=6}
result<-acc_path(graph = graph$graph, scoord = c(10.5, 10.5), costsurf = costsurf)
...
xmx = dim(x)[1], ymn = 0, ymx = dim(x)[2]) #neccessary to set resolution
  costsurf[] <- x
  costsurf
}
...
times <- lapply(1:length(dm), function(x) system.time(irl_acc(dm[[x]], poicoords = matrix(c(10.5, 10.5), ncol = 2), grainprop = 0.25, costsurf = costsurfs[[x]], scoord = c(10.5, 10.5), cutoff = 0.2)))

fulltimes <- lapply(1:length(dm), function (x) system.time(irl_acc(dm[[x]], poicoords = matrix(c(10.5, 10.5), ncol = 2), costsurf = costsurfs[[x]], scoord = c(10.5, 10.5), irregular = FALSE)))

regtimes <- lapply(costsurfs, function(x) system.time(gdist_acc(x, scoord = c(10.5, 10.5))))
...
count_vertedge <- function(graph){
  igraph::vcount(graph$graph) * igraph::ecount(graph$graph) #ethcode is not a true triangulation but instead fully diagonal...
}

vertxedges <- lapply(dm, function(x) count_vertedge(irl_graph(x, grainprop = 0.25, cutoff = 0.2)))
fullvertxedges <- lapply(dm, function(x) count_vertedge(irl_graph(x, irregular = FALSE)))
regvertxedges <- lapply(costsurfs, function(x) count_vertedge(gdist_acc(x, scoord = c(10.5, 10.5))))
#x<-costsurfs[[1]]

par(mar=c(4,4,2,2))
plot(cbind(fullvertxedges, fulltimes), ylim = c(0.1, 60), xlim = c(0, 8e+08), log = "y", ylab = "Processing Times (s)", xlab = "Landscape graph vertices x edges")
points(cbind(vertxedges, times), col = "red")
points(cbind(regvertxedges, regtimes), col = "grey")

df <- data.frame(vertxedges = unlist(vertxedges), times, fullvertxedges = unlist(fullvertxedges), fulltimes, regvertxedges = unlist(regvertxedges), regtimes)

fit <- nls(times ~ b * vertxedges^power, data = df, start = list(b = 0.001, power = 1))
fullfit <- nls(fulltimes ~ b * fullvertxedges^power, data = df, start = list(b = 0.001, power = 1))
regfit <- nls(regtimes ~ b * regvertxedges^power, data = df, start = list(b = 0.001, power = 0.2))
...
legend("topright", legend = c("full","irregular", "regular"), col = c("black", "red", "grey"), pch = 1)
