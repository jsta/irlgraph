...
graph <- suppressMessages(irl_graph(dm = dm, poicoords, grainprop = 0.25))
...
par(mar = c(0,0,3,0))
plot(costsurf, box = FALSE, axes = FALSE, legend.args = list(text="Cost"), col = rev(viridis::magma(255)))
...
```{r eval=TRUE, fig.align='left', fig.height=3.2, fig.width=3.2}
par(mar=c(2,0,0,0))
...
par(mar=c(0,0,3,0))
plot(result, box = FALSE, axes = FALSE, legend.args = list(text = "Acc. Cost"), col = rev(viridis::viridis(255)))
points(10.5, 10.5)
```
...
```{r eval=TRUE, fig.align='left', fig.height=3.2, fig.width=4.6}
...
#10 nodes
  coordmat <- matrix(c(rbind(seq(0.5, 2.5, 0.5), 0.5),
  ...
  times <- lapply(1:length(dm), function(x) system.time(irl_acc(dm[[x]], 
    poicoords = matrix(c(10.5, 10.5), ncol = 2), grainprop = 0.25,
    costsurf = costsurfs[[x]], scoord = coordmat, cutoff = 0.2)))
  
  fulltimes <- lapply(1:length(dm), function (x) 
    system.time(irl_acc(dm[[x]], poicoords = matrix(c(10.5, 10.5),
    ncol = 2), costsurf = costsurfs[[x]], scoord = coordmat, 
    irregular = FALSE, warn = FALSE)))
  
  regtimes <- lapply(costsurfs, function(x) system.time(gdist_acc(x,
    scoord = coordmat)))
  ...
  fullvertxedges <- lapply(dm,
    function(x) count_vertedge(irl_graph(x,
    irregular = FALSE)))
  regvertxedges <- lapply(costsurfs,
    function(x) count_vertedge(gdist_acc(x,
    scoord = matrix(c(10.5, 10.5), ncol = 2,
    byrow = TRUE))[[1]]))
  
  par(mar=c(4,4,2,2))
  plot(cbind(unlist(fullvertxedges), fulltimes), ylim = c(0.2, 60),
    xlim = c(0, 8e+08), log = "y", ylab = "Processing Times (s)",
...
```{r eval=TRUE, fig.align='left', fig.height=3.2, fig.width=6.4}
library(raster)
library(ggplot2)
