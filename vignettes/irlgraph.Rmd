...
resolution = 1, xmn = 0, xmx = dim(dm)[1], ymn = 0, ymx = dim(dm)[2]) #neccessary to set resolution
costsurf[] <- dm
...
poicoords = matrix(c(10.5, 10.5), ncol = 2), grainprop = 0.25, costsurf = costsurfs[[x]],
scoord = c(10.5, 10.5), cutoff = 0.2)))

fulltimes <- lapply(1:length(dm), function (x) system.time(irl_acc(dm[[x]],
poicoords = matrix(c(10.5, 10.5), ncol = 2), costsurf = costsurfs[[x]],
scoord = c(10.5, 10.5), irregular = FALSE, warn = FALSE)))

regtimes <- lapply(costsurfs,
function(x) system.time(gdist_acc(x, scoord = c(10.5, 10.5))))
...
par(mar=c(4,4,2,2))
plot(cbind(unlist(fullvertxedges), fulltimes), ylim = c(0.05, 60), xlim = c(0, 8e+08), 
  log = "y", ylab = "Processing Times (s)", xlab = "Landscape graph vertices x edges")
...
fullfit <- nls(fulltimes ~ b * fullvertxedges^power,
  data = df, start = list(b = 0.001, power = 1))
regfit <- nls(regtimes ~ b * regvertxedges^power,
  data = df, start = list(b = 0.001, power = 0.2))
...
legend("topright", legend = c("full","irregular", "regular"),
